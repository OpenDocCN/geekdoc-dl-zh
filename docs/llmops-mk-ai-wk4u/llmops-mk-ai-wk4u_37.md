...
> 原文：[`boramorka.github.io/LLM-Book/en/CHAPTER-2/Answers%202.7/`](https://boramorka.github.io/LLM-Book/en/CHAPTER-2/Answers%202.7/)

## 理论

1.  对话记忆：为聊天机器人提供消息之间的上下文，使答案更加个性化和连贯。

1.  `ConversationBufferMemory`：存储整个对话历史，以便模型可以参考当前对话中的先前轮次。

1.  对话检索链：结合记忆和从外部来源检索以改进答案的准确性和相关性。

1.  环境管理策略：从固定缓冲区到通过文档检索动态扩展环境；选择取决于任务。

1.  命名实体识别（NER）：有助于跟踪对话中的关键实体并保持讨论的完整性。

1.  数据隐私：需要最小化数据收集，匿名化敏感信息，并拥有透明、合法的数据保留政策。

1.  主题转换：当主题改变时，总结、主题感知记忆和选择性地检索最相关的历史信息可以有所帮助。

1.  评估指标：包括用户满意度、任务成功以及自动化的连贯性和相关性度量。

1.  持久记忆：有助于在会话之间保持上下文，保留用户偏好以及关于过去问题和解决方案的信息。

1.  实际建议：确保隐私和透明度，让用户控制记忆，并持续监控交互质量。
